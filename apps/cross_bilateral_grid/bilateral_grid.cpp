...
int main(int argc, char **argv) {
    ...
    ImageParam input_values(Float(32), 2);
    ImageParam input_features(Float(32), 2);
    Param<float> r_sigma("r_sigma");
    int s_sigma = atoi(argv[1]);
    Var x("x"), y("y"), z("z"), c("c");

    // Add a boundary condition
    Func clamped_features = BoundaryConditions::repeat_edge(input_features);
    Func clamped_values = BoundaryConditions::repeat_edge(input_values);

    // Construct the bilateral grid
    RDom r(0, s_sigma, 0, s_sigma);
    Expr feature_val = clamped_features(x * s_sigma + r.x - s_sigma/2, y * s_sigma + r.y - s_sigma/2);
    Expr val = clamped_values(x * s_sigma + r.x - s_sigma/2, y * s_sigma + r.y - s_sigma/2);
    feature_val = clamp(feature_val, 0.0f, 1.0f);
    Expr zi = cast<int>(feature_val * (1.0f/r_sigma) + 0.5f);
    ...
    */
    ...
    blurz(x, y, z, c) = histogram(x, y, z, c);
    blurx(x, y, z, c) = blurz(x, y, z, c);
    blury(x, y, z, c) = blurx(x, y, z, c);

    // Take trilinear samples to compute the output
    feature_val = clamp(input_features(x, y), 0.0f, 1.0f);
    Expr zv = feature_val * (1.0f/r_sigma);
    zi = cast<int>(zv);
    Expr zf = zv - zi;
    ...
    bilateral_grid.compile_to_file("bilateral_grid", {r_sigma, input_values, input_features}, target);
